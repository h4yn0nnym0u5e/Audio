...
//======================================================================================================
const char* names[] = {"wm_tell4.wav","wm_tell6.wav","wm_tell8.wav", "wmt4copy.wav"};
unsigned long last_time = millis();
enum state_e {boot,synth,playonly,playback,playing,peace} state = playonly;
enum copier_e {wait,opened,copying,done,failed} copystate;
//-------------------------------------------------
/*
...
//-------------------------------------------------
// File copier. Prove we can use the SD card 
// during playback. If we're polling the events
// ourselves using AudioEventResponder::runPolled(),
// then the ::(dis|en)ableResponse() calls can be
// safely removed.
//
void updateCopier(void)
{
  static File src,dst;
  static int copied = 0;
  static int blks;
  ...
  switch(copystate)
  {
    ...
    case wait:
      if (playing == state) // we have a file to copy
      {
        AudioEventResponder::disableResponse();
        src = SD.open(names[0],FILE_READ);
        dst = SD.open(names[3],FILE_WRITE_BEGIN);
        dst.truncate();
        ...
        if (src & dst)
        {
          copystate = opened;
          blks = 0;
          Serial.printf("Opened files to copy: %s -> %s\n", 
                        src.name(), dst.name());
        }
        else
        {
          copystate = failed;
          Serial.println("Failed to open files to copy");
        }
      }
      break;
    ...
    case opened:
    {
      char buf[2017];  // be deliberately awkward
      int got;
