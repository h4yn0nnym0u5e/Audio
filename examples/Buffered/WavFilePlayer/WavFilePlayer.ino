...
void setup() {
  ...
  // Buffered playback object requires a buffer to be allocated. Pick one of these:
  // playWav1.createBuffer(4096,AudioBuffer::inHeap); // everything has heap memory
...
void playFile(const char *filename)
{
  ...
  // is NOT needed for the buffered playback objects
  // delay(25);

  // Simply wait for the file to finish playing.
  while (playWav1.isPlaying()) 
  {
    yield();    // loop() isn't exiting, so allow EventResponder to re-load the buffers
    delay(100); // this works as well - use either
